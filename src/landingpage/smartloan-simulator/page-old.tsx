  ...
  const calculateLoan = () => {
    ...
    if (interestType === "flat") {
      ...
      if (floatingMonths > 0) {
        ...
        const remainingPrincipalForFloating = balance;
        const floatingMonthlyPayment = (remainingPrincipalForFloating * (floatingMonthlyRate * Math.pow(1 + floatingMonthlyRate, floatingMonths))) / (Math.pow(1 + floatingMonthlyRate, floatingMonths) - 1);

        for (let i = 1; i <= floatingMonths; i++) {
          const interestPaid = balance * floatingMonthlyRate;
          const principalPaid = floatingMonthlyPayment - interestPaid;
          balance -= principalPaid;
          installments.push({ month: i + fixedMonths, payment: floatingMonthlyPayment, principal: principalPaid, interest: interestPaid, balance: Math.max(0, balance) });
        }
      }
      // Use average payment for DTI on floating loans
        ...
        if (mainCanvas) {
          mainChartRef.current = new (window as any).Chart(mainCanvas.getContext("2d"), {
            ...
            options: {
              ...
              scales: { x: { stacked: true, ticks: { color: "#94a3b8" } }, y: { stacked: true, ticks: { color: "#94a3b8", callback: (v: number) => formatCurrency(Number(v)).replace("Rp ", "") + " Jt" } } },
            },
          });
        }
        if (donutCanvas) {
    ...
    <main className="min-h-screen bg-slate-900 text-slate-200 p-4 md:p-8 font-sans">
      ...
      <div className="container mx-auto max-w-7xl">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          <h1 className="text-3xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-teal-400">Smart Loan Simulator</h1>
          <p className="mt-3 text-lg text-slate-400">Rencanakan pinjaman Anda dengan presisi dan pemahaman mendalam.</p>
        </motion.div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-1 bg-slate-800/60 p-6 rounded-2xl border border-slate-700 h-fit space-y-6">
            <ControlGroup title="Jenis Pinjaman & Bunga">
              <select value={loanType} onChange={(e) => setLoanType(e.target.value)} className="w-full p-2.5 rounded-lg bg-slate-700 border border-slate-600 focus:outline-none focus:ring-2 focus:ring-cyan-500">
                {loanTypes.map((lt) => (
              ...
              <select
                ...
                className="w-full p-2.5 rounded-lg bg-slate-700 border border-slate-600 focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
            ...
            <ControlGroup title="Struktur Pinjaman">
              ...
              <div className="text-sm bg-slate-900/50 p-3 rounded-lg text-center">
                <p className="text-slate-400">Pokok Pinjaman</p>
              ...
              <AnimatePresence>
                {interestType === "effective" && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="space-y-4 overflow-hidden">
                    ...
                    <SliderControl label="Suku Bunga Floating" value={floatingRate} onChange={setFloatingRate} min={5} max={25} step={0.1} displayValue={`${floatingRate.toFixed(1)}% / tahun`} />
                  </motion.div>
                )}
              </AnimatePresence>
            </ControlGroup>
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full mt-2 bg-gradient-to-r from-cyan-500 to-teal-500 text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 transition disabled:opacity-50"
            >
              {isCalculating ? <FaSpinner /> : <FaCalculator />}
            ...
            <AnimatePresence>
              {!result && !error && (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="flex items-center justify-center h-full bg-slate-800/60 p-10 rounded-2xl border border-slate-700 border-dashed"
                >
              ...
              {result && (
                <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                  ...
                  <div className="bg-slate-800/60 p-6 rounded-2xl border border-slate-700">
                    <h3 className="text-xl font-bold text-cyan-300 mb-4">Grafik Amortisasi (Pokok vs Bunga)</h3>
...
const ControlGroup: FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <div className="border-t border-slate-700 pt-5 first-of-type:border-t-0 first-of-type:pt-0">
  ...
  <div>
    ...
    <div className="relative">
      ...
      <input
        ...
        className="w-full pl-8 p-2 rounded-lg bg-slate-700 border border-slate-600 focus:outline-none focus:ring-2 focus:ring-cyan-500"
      />
    </div>
  </div>
);
const SliderControl: FC<{ label: string; value: number; onChange: (val: number) => void; min: number; max: number; step: number; displayValue: string }> = ({
  ...
  <div>
    <div className="flex justify-between items-center text-xs text-slate-400 mb-1">
      <label>{label}</label>
      <span className="font-semibold text-cyan-400 bg-slate-700 px-2 py-0.5 rounded">{displayValue}</span>
    </div>
    <input
      type="range"
      min={min}
...
const ResultStat: FC<{ label: string; value: string; dti?: number; className?: string }> = ({ label, value, dti, className }) => {
  ...
  return (
    <div className={`bg-slate-800 p-4 rounded-lg text-center ${className}`}>
...
const ResultCards: FC<{ result: Result; loanAmount: number }> = ({ result, loanAmount }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div className="bg-slate-800/60 p-6 rounded-2xl border border-slate-700">
      <h2 className="text-xl font-bold text-cyan-300 mb-4">Ringkasan Finansial</h2>
    ...
    <div className="bg-slate-800/60 p-6 rounded-2xl border border-slate-700">
      <h3 className="text-xl font-bold text-cyan-300 mb-4">Komposisi Pembayaran</h3>
      <div className="h-48">
